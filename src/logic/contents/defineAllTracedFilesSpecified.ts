...
export const defineAllTracedFilesSpecified = async ({
  ...
  const filesToTrace = await defineAllPickedFilesSpecified({
    projectRootDirectory,
    pickFileGlobs: traceFileGlobs,
  });

  // split the files between the ones we need to trace for + the ones that are already trace outputs
  const filesToRunTracingFor = filesToTrace.filter((fileToTrace) =>
    fileToTrace.endsWith('.js'),
  ); // if it ends with .js, we need to trace it _and_ follow the dependencies
  const filesToUseTracingOf = filesToTrace.filter((fileToTrace) =>
    fileToTrace.endsWith('.nft.json'),
  ); // if it ends with .nft.json, it is already the output of tracing and we can just follow the dependencies
  ...
  const filesNotCapableOfTracing = filesToTrace.filter(
    (fileToTrace) =>
      !filesToRunTracingFor.includes(fileToTrace) &&
      !filesToUseTracingOf.includes(fileToTrace), // if its not included in either of the two lists, then we cant run tracing for it
  );
  if (filesNotCapableOfTracing.length)
    throw new UserInputError(
      'files specified for tracing on which tracing can not be run',
      {
  ...
  const filesTracedByRunningTracing = (
    await nodeFileTrace(
      filesToRunTracingFor.map(
        (relativePath) => `${projectRootDirectory}/${relativePath}`,
      ),
      {

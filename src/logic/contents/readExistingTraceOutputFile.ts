...
export const readExistingTraceOutputFile = async ({
  ...
  if (!contents.files || !Array.isArray(contents.files))
    throw new UserInputError(
      `invalid trace output file detected: ${existingTraceOutputFilePath}`,
      {
        potentialSolution:
  ...
  const traceOutputFileDirectory = absolutePathOfTraceOutputFile
    ...
    .join('/');
  const absolutePathsOfTracedFiles =
    tracedFilePathsRelativeToTraceOutputFile.map((tracedFilePath) =>
  ...
  const tracedFilePaths = absolutePathsOfTracedFiles.map(
    (absoluteTracedFilePath) =>
      absoluteTracedFilePath.replace(`${projectRootDirectory}/`, ''), // remove the prefix of the project dir
  );
